...
ABOUT_TEXT_2            EQU 'v2.03.00 for Windows x64.'
...
stosq                              ; Store pointer to icon resource                 
...
push 0                            ; This PUSH 0 for stack alignment
...
lea rcx,[NAME_INF]
lea rdx,[rdi + APPBUFFERS.handleLoadInf]
lea r8,[rdi + APPBUFFERS.pointerLoadInf]
...
mov r8,[r15 + APPDATA.lockedConfigInfo]
lea r9,[rdi + APPBUFFERS.statusLoadInf_1] 
test rdx,rdx
jz .skipInf
cmp rdx,LOAD_INF_LIMIT
jae .errorInfBig
add rbp,rdx
test rbp,0Fh
jz @f
...
stosw
mov rdx,rdi
mov rcx,[APP_DATA.lockedConfigInfo]
call ShowInf
mov r9d,MB_ICONINFORMATION   ; R9  = Parm#4 = Message box icon type
lea r8,[PROGRAM_NAME]        ; R8  = Parm#3 = Pointer to caption
pop rdx rdi                  ; RDX = Parm#2 = Pointer to string
xor ecx,ecx                  ; RCX = Parm#1 = Parent window handle or 0
call [MessageBoxA]
...
not rdx
and rax,rdx
mov cl,1      ; Set CL = 1 if RAX updated by alignment restriction
@@:
cmp rax,CUSTOM_BLOCK_MIN
jae @f
mov rax,CUSTOM_BLOCK_MIN
mov cl,1      ; Set CL = 1 if RAX updated by minimum block size restriction
@@:
cmp rax,CUSTOM_BLOCK_MAX
jbe @f
mov rax,CUSTOM_BLOCK_MAX
mov cl,1      ; Set CL = 1 if RAX updated by maximum block size restriction
@@:
...
mov rax,[BIND_LIST.getBlkCustom]
call CustomBlockSizeRestrictions
jnc .exit                                ; Go skip if no restriction checks
mov [BIND_LIST.getBlkCustom],rax
mov ax,STR_CUSTOM_ERROR
lea rdi,[TEMP_BUFFER]  ; RDI = Pointer to buffer for build message
mov rdx,rdi            ; RDX = Parm#2 = Pointer to message string, copy
call PoolStringWrite
mov ax,0A0Dh
stosw
...
mov ax,0000h + '.'
stosw
mov r9d,MB_ICONWARNING  ; R9  = Parm#4 = Message type 
...
mov r8d,17             ; R8  = Parm#3 = Maximum length
mov edx,WM_GETTEXT     ; RDX = Parm#2 = Message
xchg rcx,rax           ; RCX = Parm#1 = GUI item handle
call [SendMessage]
test rax,rax
jz .error              ; Go if error, can't read string
mov byte [rsi + 16],0  ; Safe terminator byte, limit scan pointer position
xor ecx,ecx            ; RCX = 0, used for accumulate extracted value
test r13b,r13b
jz .parseDec
...
test r13d,r13d         ; Detect decimal or hexadecimal mode 
jz .errorMessage 
...
include 'ncrb64\load_kernel_mode_driver\connect_code.inc'
...
APP_MEM_ALLOC  DQ  0                        ; Must be 0 for conditional release
...
include 'ncrb64\gui_dialogs\connect_data.inc'
include 'ncrb64\load_config\connect_data.inc'
include 'ncrb64\load_kernel_mode_driver\connect_data.inc'
...
statusLoadInf_1  dq  ?     ; Pointer to status string 1 if NCRB.INF parse error 
statusLoadInf_2  dq  ?     ; Pointer to status string 2 if NCRB.INF parse error
...
lockedDataSmbios        dq ?     ; Data base for SMBIOS structures detection
...
lockedConfigInfo        dq ?     ; Strings for parsing configuration INF file
...
; One extra element required because N points means N-1 Polyline intervals
polylineArray    dq  ?    
                 dq  DRAW_POINTS_COUNT dup (?)
ends
align 8
