...
16.    Experiments with threads priority.
17.    Experiments with visualization timer refresh rate.
18.    Memory fence required for measurement and visualization threads shared variables.
...
3.    Compare results for v2.01.xx and v2.02.xx. Y-values and X-points:
       point for performance change = cache size.
4.    Inspect for unused objects, remove it.
5.    Skip re-scaling if new maximum - old maximum < threshold.
6.    Inspect and work with "TODO" strings in the source text.
7.    Optimize statistics calculation for add one result case.
8.    Note about "Draw 3D..." button restore, probably at Draw window.
